...
class GroundTruthChannel:
    ...
    def __init__(self,
        ...
        bool_array = bool_array[:,:,0].squeeze()
...
class AnomalyMap:
    ...
    Use AnomalyMap.read_from_png(...) to read from a .png file.
    """
    ...
    @classmethod
    def read_from_png(cls, png_path: str, resize_shape: Optional[Tuple[int,int]] = None):
        """Read an AnomalyMap from a PNG file."""
        image = Image.open(png_path)
        if resize_shape is not None:
            image = image.resize(resize_shape)
        np_array = np.array(image)
        if len(np_array.shape) != 2:
            np_array = np.array[:,:,0].squeeze()
        assert len(np_array.shape) == 2
