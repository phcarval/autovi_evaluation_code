"""
Evaluate a single experiment on a single object of the AutoVI dataset.
...
def main():
    ...
    resize_shape = set_resize_shape(args.object_name)
    ...
    anomaly_maps_test_dir = args.anomaly_maps_dir
    ...
    localization_results = {'auc_spro': get_auc_spros_for_metrics(
        metrics=metrics)}
...
def get_available_gt_map_rel_paths(gt_dir: str) -> Iterable[str]:
    ...
    for defect_type_name in listdir(gt_dir):
        ...
        if defect_type_name in valid_defect_type_names:
            raise OSError(
...
def get_auc_spro_results(metrics: ThresholdMetrics,
    ...
    if not weighted_mean:
        ...
        for s in subdir_names:
            if '.png' not in s:
                nb_images[s] = len(listdir(os.path.join(anomaly_maps_test_dir, s)))
        
        total = sum(nb_images.values())

        ratio_images = {k:v / total for k,v in nb_images.items()}

    # Compute the mean performance over logical and structural anomalies.
    mean_spros = dict()
    classes = list(auc_spro.keys())
    for limit in auc_spro[classes[0]].keys():
        sum_auc = None
        if weighted_mean:
            sum_auc = sum([auc_spro[c][limit] for c in classes])
            sum_auc = sum_auc / len(classes)
        else:
            sum_auc = sum([auc_spro[c][limit] * ratio_images[c] for c in classes])
        mean_spros[limit] = sum_auc
...
def get_auc_spros_per_subdir(metrics: ThresholdMetrics,
    ...
    subd = subdir_names.copy()

    for s in subd:
        if '.png' in s:
            subdir_names.remove(s)
...
def get_image_level_detection_metrics_aggregated(per_image_results):
    ...
    for image_result in per_image_results:
        try:
            ...
            anomaly_scores[image_result['image_type']] = \
                [image_result['anomaly_scores']['max']]

    # Compute auc-roc for structural and logical anomalies separately.
    classes = anomaly_scores.keys()
    for c in classes:
        if c == 'good':
            continue
        agg_results['auc_roc'][c] = \
            compute_classification_auc_roc(
                anomaly_scores['good'], anomaly_scores[c])

    agg_results['auc_roc']['mean'] = \
        sum([agg_results['auc_roc'][c] for c in classes if c != 'good']) / (len(classes) - 1)
    return agg_results
